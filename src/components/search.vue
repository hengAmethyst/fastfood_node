<template>
	...
	<div class="search">
		...
		<div class="inner" v-show="searchData.length<1">
		...
		<div class="goods">
			<ul>
				<li v-for="(item,index) in searchData">
					<p>
						<img @click="showFoodImg(item,index)" :src="item.picUrl"/>
					</p>
					<p>
						<span>{{item.name}}</span>
		...
		<!--菜品大图-->
		<foodImg class="foodImg" :food="temp" ref="foodImg"></foodImg>
	</div>
</template>
<script>
import detailPage from './order/buy/detailPage.vue'
import foodImg from './order/buy/foodImg.vue'
	export default{
		data(){
			return{
				iFlag:true,
