<template>
	<div class="register">
		...
		<!--出生日期-->
		<div class="birthday">
			<input type="text" v-model="birthday" placeholder="1990-01-01" @focus="openPicker" ref="birTime"/>
		</div>
		<!--性别-->
		<div class="gender">
			<div class="genderBox">
...
Vue.component('c-datetime-picker',DatetimePicker)
	...
	export default{
		...
		data(){
			return{
				information:{name:'激活会员',ifShowReload:false,ifShowMenu:false},
				radio: '-1',
				//暂时存储手机号码
				phoneNumber:null,
				//控制验证码显示
...
//				activeBtn:true,
		...
		methods:{
			...
			//提交
			registerSubmit(){
				this.$http.post(this.$store.state.accountHttp+'/register',
							JSON.stringify(
						...
						.then(function(res){
							...
							if(res.body.code == 5006){
								...
								this.$http.post(this.$store.state.accountHttp+'/queryById',
									JSON.stringify({
													"openId":this.$store.state.openId,
													"merchantId":this.$store.state.merchantId
													})
